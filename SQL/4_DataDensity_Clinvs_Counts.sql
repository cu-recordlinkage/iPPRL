...
, linked_sids as (
    ...
    , enc_key as (
    	select row_number() over() as enc_id, c.* from :cumulative_to_use c
    )
    , clinvs_lastrun as (
        select distinct run_to_analyze, last_nid, c.study_id as sid, enc_id, startdate, enddate, enddate2, encountertype as enc_type
            , providertype as prov_type, heightinches as hgt, weightpounds as wgt, siteid
        from linked_sids_lastrun lsr join enc_key c on (lsr.sid = c.study_id)
    ...
    select 'Patient/Encounter' as type
        , run_to_analyze
    ...
    union 
    select 'Network/Encounter' as type
        , run_to_analyze
        , round(avg(n_startdt),2) as startdt_dd
        , round(avg(n_enddt),2) as enddt_dd
        , round(avg(n_enc_type),2) as enc_type_dd
